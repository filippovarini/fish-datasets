...
annotations_path = (
    settings.raw_dir / DATASET_SHORTNAME / "coco/coralscapes-coco/coralscapes.json"
)
...
def processing():
    ...
    copy_images_to_processing(
        DATASET_SHORTNAME,
        settings.raw_dir / DATASET_SHORTNAME / "coco/coralscapes-coco",
    )
...
def dataset_splitting():
    ...
    should_the_image_be_included_in_train_set = (
        ...
        lambda image_path: int(Path(image_path).stem.split("_")[1].split("e")[1])
        < 25
    )
