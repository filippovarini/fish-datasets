...
annotations_path = processing_dir / settings.coco_file_name
...
def processing():
    ...
    copy_images_to_processing(
        DATASET_SHORTNAME, settings.raw_dir / DATASET_SHORTNAME / "images"
    )

    shutil.copy(
        str(settings.raw_dir) + f"/{DATASET_SHORTNAME}/annotations.json",
        annotations_path,
    )
...
def dataset_splitting():
    ...
    should_the_image_be_included_in_train_set = (
        ...
        lambda image_path: Path(image_path).stem.split("_")[-1]
        == "train"
    )
