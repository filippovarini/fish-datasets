...
extract_dir = label_dir = data_folder / 'f4k_detection_tracking'
...
def extract_data():
    ...
    for filename in os.listdir(extract_dir):
        if filename.endswith(".flv"):
            ...
            subprocess.run(cmd, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL) # delete last two parameters to view ffmpeg conversion output
...
def extract_keyframes(label_path, video_path, video_name, category_id_map, annotation_id, image_id):
    ...
    for frame in root.findall("frame"):
        ...
        frame_id = int(frame.attrib["id"]) + video_info[video_id][1]  # Frame id = frame reference in the xml + offset 
        ...
        subprocess.run(cmd, stderr=subprocess.DEVNULL) # delete last parameter to view ffmpeg output
...
def clean_annotations():
    ...
    for video_name in video_names:
        ...
        coco_dict, annotation_id, image_id = extract_keyframes(label_path, video_path, video_name, category_id_map, annotation_id, image_id)
...
if __name__ == "__main__":
    ...
    save_sample_image("f4k", image_sample)
