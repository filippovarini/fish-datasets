...
The script reads the Parquet files, extracts images to files,
...
output_preview_folder = input_folder / "coco/coralscapes-coco-preview"


# Rendering and segmentation functions
def overlay_segmentation(
    img, label, category_id_to_name, category_name_to_color, alpha=0.5
):
    """
...
def get_bounding_boxes(
    mask_image, image_id, include_category_ids=None, exclude_category_id=None
):
    """
    ...
    if exclude_category_id is not None:
        if exclude_category_id in unique_categories:
            unique_categories = unique_categories[
                unique_categories != exclude_category_id
            ]
    ...
    for category_id in unique_categories:
        ...
        labeled_mask, num_labels = measure.label(
            mask_binary, return_num=True, connectivity=2
        )
...
def integrity_check():
    ...
    options.baseDir = str(output_image_folder)
    options.bCheckImageSizes = True
    options.bCheckImageExistence = True
    options.bFindUnusedImages = True
    options.bRequireLocation = True

    _ = integrity_check_json_db(str(output_coco_file), options=options)
...
def create_coco_dataset():
    ...
    output_dict["info"][
        "description"
    ] = "Coralscapes dataset, fish only, converted to boxes"
    ...
    for i_file, fn_relative in enumerate(parquet_files_relative):
        ...
        for i_row, row in tqdm(df.iterrows(), total=len(df)):
            ...
            if False:
                im_out = overlay_segmentation(
                    img, label, category_id_to_name, category_name_to_color, alpha=0.5
                )
                target_width = 500
                im_out = im_out.resize(
                    (target_width, int(im_out.height * target_width / im_out.width)),
...
def visualize_preview():
    ...
    html_filename, _ = visualize_db(
        db_path=str(output_coco_file),
        output_dir=str(output_preview_folder),
        image_base_dir=str(output_image_folder),
    )
